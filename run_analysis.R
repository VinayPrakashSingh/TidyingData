...
## import labels
labels <- read.table(paste(data_dir, "activity_labels.txt", sep="/"), 
                     col.names=c("labelcode","label"))
## import features
features <- read.table(paste(data_dir, "features.txt", sep="/"))[,2]
# Make logical vector with mean & std set to TRUE.
goodfeatures <- grepl("mean|std", features)
# Load and process X_test & y_test data.
training_subject <- read.table(paste(data_dir, "train", "subject_train.txt", sep="/"),
                               col.names = "subject")
...
training_labels <- read.table(paste(data_dir, "train", "y_train.txt", sep="/"),
                              col.names = "labelcode")
